...
# for any error
def error(message):
    print(Fore.RED + f"An error occurred because  {message}" + Style.RESET_ALL)
...
def chooseImage():
    ...
    for f in files:
        if f.endswith(".jpg"):
            myFiles.append(f)
            print(Fore.MAGENTA + f"{count}_ {f}" + Style.RESET_ALL)
            count += 1
    image = input(Fore.CYAN +"Enter the image name number: "+ Style.RESET_ALL)
...
def code_text(im, data, change_event):
    text_data = data
    ticket_needed = input(
        Fore.CYAN + "Do you want to add a text code (Y/N): " + Style.RESET_ALL
    )
    ...
    if ticket_needed.upper() == "Y" and (change_event == True or text_data == {}):
        color_options = input(
            Fore.CYAN + "Do you want the color black or white (B/W): " + Style.RESET_ALL
        )
        ...
        ticket_needed = True
        text_data["x"] = input(
            Fore.CYAN + "Enter the text x-access: " + Style.RESET_ALL
        )
        while (
            text_data["x"] == ""
        ...
        text_data["y"] = input(
            Fore.CYAN + "Enter the text y-access: " + Style.RESET_ALL
        )
        while (
            text_data["y"] == ""
        ...
        text_data["size"] = input(Fore.CYAN + "Enter the text size: " + Style.RESET_ALL)
        while (
            text_data["size"] == ""
            ...
            text_data["size"] = input(
                Fore.CYAN + "Enter the text size: " + Style.RESET_ALL
            )
        text_data["ticket_needed"] = ticket_needed
        return text_data
...
events = get_events()
print(Fore.CYAN + "Choose the event number: " + Style.RESET_ALL)
for i in range(len(events)):
    print(Fore.MAGENTA + f'{i+1}_ {events[i]["name"]}' + Style.RESET_ALL)
event = input(Fore.CYAN +"Enter the event number: "+ Style.RESET_ALL)
while event == "" or event.isnumeric() == False or int(event) > len(events):
    error(Fore.RED + "This event is not a valid event number" + Style.RESET_ALL)
    event = input(Fore.CYAN +"Enter the event number: "+ Style.RESET_ALL)
...
change_event = input(
    Fore.CYAN + "Do you want to change the event coordination (Y/N): " + Style.RESET_ALL
)
while change_event == "" or change_event.upper() not in ["Y", "N"]:
    error("This is not a valid answer")
    change_event = input(
        Fore.CYAN
        + "Do you want to change the event coordination (Y/N): "
        + Style.RESET_ALL
    )
...
type = input(Fore.CYAN + "Enter the type of this codes: " + Style.RESET_ALL)
while type == "" or type == type.isnumeric() == False or int(type) > len(types):
    error("This type is not a valid type")
    type = input(Fore.CYAN + "Enter the type of this codes: " + Style.RESET_ALL)

# for QR code data
isQRcode = input(Fore.CYAN + "Do you need a QR code? (Y/N): " + Style.RESET_ALL)
...
if isQRcode.upper() == "Y" and (change_event == True or qr_data == {}):
    qr_data["x"] = input(Fore.CYAN + "Enter the qrcode x-access: " + Style.RESET_ALL)
    while qr_data["x"] == "" or qr_data["x"].isnumeric() == False:
        error("This x access is not a valid number")
        qr_data["x"] = input("Enter the qrcode x-access: ")
    qr_data["y"] = input(Fore.CYAN + "Enter the qrcode y-access: " + Style.RESET_ALL)
    while qr_data["y"] == "" or qr_data["y"].isnumeric() == False:
        error("This y-access is not a valid number")
        qr_data["y"] = input(
            Fore.CYAN + "Enter the qrcode y-access: " + Style.RESET_ALL
        )
    qr_data["border"] = input(
        Fore.CYAN + "Enter the border size of QR-code: " + Style.RESET_ALL
    )
    if qr_data["border"] == "" or qr_data["border"].isnumeric() == False:
        qr_data["border"] = 0
    else:
        qr_data["border"] = int(qr_data["border"])
    qr_data["size"] = input(Fore.CYAN + "Enter the size of QR-code: " + Style.RESET_ALL)
    if (
        qr_data["size"] == ""
...
tickets = input(Fore.CYAN +"Enter the number of tickets to generate: "+ Style.RESET_ALL)
...
server_sent = input(Fore.CYAN +"Do you want to send tickets to the server (Y/N): "+ Style.RESET_ALL)
while server_sent == "" or server_sent.upper() not in ["Y", "N"]:
    error("This is not a valid answer")
    server_sent = input(Fore.CYAN +"Do you want to add a text code (Y/N): "+ Style.RESET_ALL)
...
if server_sent.upper() == "N":
    ...
    print(
        Fore.YELLOW
        + "---------------- i'm sending the data to the server ----------------"
        + Style.RESET_ALL
    )

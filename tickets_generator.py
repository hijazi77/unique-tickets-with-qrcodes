...
def code_text(im):
    ...
    if ticket_needed.upper() == "N":
        ...
        while (
            text_size == ""
            or text_size.isnumeric() == False
            or int(text_size) > im.size[1]
            or int(text_size) > im.size[0]
            or int(text_size) < 0
        ):
...
def counter_ticket(im):
    ...
    if is_counter.upper() == "Y":
        ...
        while (
            counter_size == ""
            or counter_size.isnumeric() == False
            or int(counter_size) > im.size[1]
            or int(counter_size) > im.size[0]
            or int(counter_size) < 0
        ):
            error("This text size is not a valid number")
            counter_size = input("Enter the text size: ")
...
def generate():
    for i in tqdm(
        ...
        if ticket_counter != False:
            ...
            font = ImageFont.truetype("arial.ttf", int(ticket_counter["size"]))
            draw.text(
                (int(ticket_counter["x"]), int(ticket_counter["y"])),
                f"{i+1}",
                font=font,
                fill=text[4],
            )
        ...
        if text[0] == True:
            draw = ImageDraw.Draw(src)
            draw.text(
                (int(text[1]), int(text[2])), f"{pas}", font=fontcode, fill=text[4]
            )  # code
...
if server_sent.upper() == "N":
    ...
    r = requests.post("{base_url}api/add", json={"code": list})
    print(r.json())
    print("---------------- i'm done here ----------------")
